class gameplayScene15 extends Phaser.Scene {
    constructor() {
        super('gameplay15');
    }
    ...
    preload() {
        ...
        // PALET KAMBOJA: Biru, Merah, Putih (Pake aset yang udah ada)
        ...
        this.load.image('flagwin_lvl15', 'asset/flagwin_lvl15.png');
        
        // ASET LOGO KAMBOJA DARI FIGMA LU
    ...
    create() {
        ...
        // Definisi Warna Vibran Kamboja
        this.colorBlue = 0x032EA1; // Biru Kamboja
        this.colorRed = 0xE00025;  // Merah Kamboja
        this.colorWhite = 0xFFFFFF; // Putih

        // JAWABAN BENAR KAMBOJA (Biru - Merah - Biru)
        this.flagAnswers = [
            { text:"BIRU DI ATAS DAN BAWAH", top: this.colorBlue, bottom: this.colorBlue },
            { text:"MERAH DI TENGAH", mid: this.colorRed } 
        ];
        ...
        // BANK SOAL KAMBOJA
        this.hintQuestions = [
            {q:"Apa ibu kota negara Kamboja?",a:"Hanoi",b:"Phnom Penh",c:"Vientiane",d:"Bangkok",correct:"b"},
            {q:"Candi bersejarah yang ada di bendera Kamboja adalah?",a:"Borobudur",b:"Prambanan",c:"Angkor Wat",d:"Taj Mahal",correct:"c"},
            {q:"Kamboja terletak di kawasan?",a:"Asia Timur",b:"Asia Selatan",c:"Asia Tenggara",d:"Asia Barat",correct:"c"},
            {q:"Mata uang negara Kamboja adalah?",a:"Riel",b:"Baht",c:"Dong",d:"Kyat",correct:"a"},
            {q:"Negara yang berbatasan darat dengan Kamboja adalah?",a:"Thailand",b:"Malaysia",c:"Singapura",d:"Myanmar",correct:"a"}
        ];
        ...
        // RUMUS KAMBOJA (Kaya Spanyol): Biru (1/4), Merah (2/4), Biru (1/4)
        ...
        // ================= PENYUSUNAN LAYER BENDERA KAMBOJA ================= //

        // ATAS (Biru - 1/4 bagian)
        ...
        // TENGAH (Merah - 2/4 bagian / lebih tebal)
        const midY = boardY;
        ...
        // LOGO KAMBOJA EFEK MAGIC (Angkor Wat)
        // Layer 1: Logo Berwarna Putih (Sembunyi dulu, Alpha = 0)
        this.logoWarna = this.add.image(boardX, boardY, 'kamboja_logo_warna').setDepth(16).setAlpha(0).disableInteractive();
        
        // Layer 2: Logo Garis (Nongol terus)
        this.logoGaris = this.add.image(boardX, boardY, 'kamboja_logo_garis').setDepth(20).disableInteractive();

        // OUTLINE & GARIS PEMISAH MURNI KODE
        ...
        // LOGIKA MEWARNAI (Dengan deteksi isMidZone)
            ...
            const startPainting = () => {
                ...
                this.tweens.add({
                    ...
                    onComplete: () => {
                        ...
                        // --- LOGIKA REVEAL LOGO ANGKOR WAT --- //
                        if (isMidZone) {
                            // Kalau diwarnain Merah, Angkor Wat Putihnya muncul
                            if (paintColor === this.colorRed) {
                                this.tweens.add({ targets: this.logoWarna, alpha: 1, duration: 500, ease: 'Power2' });
                            } else {
                                // Kalau salah warna, balikin jadi garis doang
        ...
        // EVENT KLIK (Perhatiin isMidZone = true khusus zona tengah)
        ...
        // PALET WARNA (Biru, Merah, Putih)
        const pBiru = this.add.image(width - 90, height/1.470 - 10, 'paletBiru').setInteractive().setScale(0.8);
        ...
        const pMerah = this.add.image(width - 90, height/1.8 + 10, 'paletMerah').setInteractive().setScale(0.8);
        ...
        this.add.text(titleX, titleY, 'KAMBOJA', { fontSize: (32 * titleScale) + 'px', fontFamily: 'Arial', color: '#000000', fontStyle: 'bold' }).setOrigin(0.5);
    ...
    checkWinCondition(zoneTop, zoneMid, zoneBottom) {
        ...
        // KAMBOJA: Biru (Atas), Merah (Tengah), Biru (Bawah)
        if (zoneTop.getData('colorCode') === this.colorBlue && zoneMid.getData('colorCode') === this.colorRed && zoneBottom.getData('colorCode') === this.colorBlue) {
            this.gameOver = true; 
    ...
    showWinScreen() {
        ...
        let rewardLevel15 = localStorage.getItem('rewardLevel15');
        if(!rewardLevel15){
            let hintData = localStorage.getItem('hintData');
            let hint = hintData ? parseInt(hintData) : 0;
            hint += 1;
            localStorage.setItem('hintData', hint);
            localStorage.setItem('rewardLevel15', true);
        }
        ...
        const flag = this.add.image(flagX, flagY, 'flagwin_lvl15').setDepth(201).setScale(flagScale * 0.8).setAlpha(0);
        ...
        const unlockNextLevel = () => {
            ...
            levelData[15] = 2; 
            if (levelData[16] !== 2) levelData[16] = 1; 
        ...
        next.on('pointerdown', () => { unlockNextLevel(); this.scene.start('gameplay16'); }); // Lanjut Level 16
    }
