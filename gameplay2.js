class gameplayScene2 extends Phaser.Scene {
    ...
    preload() {
        // MUAT ASET UMUM (Bisa di-skip kalau udah dimuat di scene sebelumnya, 
        // tapi biar aman tetep kita load)
        ...
        // ASET KHUSUS LEVEL 2 (POLANDIA)
        this.load.image('flagwin_lvl2', 'asset/flagwin_lvl2.png');
    ...
    create() {
        ...
        // JAWABAN BENAR UNTUK POLANDIA (Putih Atas, Merah Bawah)
        this.flagAnswers = [
            { text:"PUTIH DI BAGIAN ATAS", top:0xFFFFFF },
            { text:"MERAH DI BAGIAN BAWAH", bottom:0xD9252B }
        ];
        ...
        // JUDUL NEGARA - UBAH JADI POLANDIA
        ...
        this.add.text(titleX, titleY, 'POLANDIA', { fontSize: (32 * titleScale) + 'px', fontFamily: 'Arial', color: '#000000', fontStyle: 'bold' }).setOrigin(0.5);
    ...
    checkWinCondition(zoneTop, zoneBottom) {
        ...
        // LOGIKA MENANG POLANDIA (ATAS PUTIH, BAWAH MERAH)
        if (topColor === 0xFFFFFF && bottomColor === 0xD9252B) {
            this.gameOver = true; 
    ...
    showWinScreen() {
        ...
        // REWARD LEVEL 2
        let rewardLevel2 = localStorage.getItem('rewardLevel2');
        if(!rewardLevel2){
            let hintData = localStorage.getItem('hintData');
            let hint = hintData ? parseInt(hintData) : 0;
            hint += 1;
            localStorage.setItem('hintData', hint);
            localStorage.setItem('rewardLevel2', true);
        }
        ...
        // TAMPILKAN BENDERA POLANDIA
        const flag = this.add.image(flagX, flagY, 'flagwin_lvl2').setDepth(201).setScale(flagScale * 0.8).setAlpha(0);
        ...
        // UPDATE LEVEL DATA: BUKA LEVEL 3
        const unlockNextLevel = () => {
            let levelDataStr = localStorage.getItem('levelData');
            let levelData = levelDataStr ? JSON.parse(levelDataStr) : {};
            levelData[2] = 2; // Level 2 tamat
            if (levelData[3] !== 2) levelData[3] = 1; // Level 3 terbuka (kalo belum tamat)
        ...
        next.on('pointerdown', () => { unlockNextLevel(); this.scene.start('gameplay3'); });
    }
