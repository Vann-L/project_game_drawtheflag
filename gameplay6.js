class gameplayScene6 extends Phaser.Scene {
    ...
    create() {
        ...
        this.flagAnswers = [
            { text:"PUTIH DI BAGIAN LATAR BENDERA", bg: 0xFFFFFF },
            { text:"MERAH DI LENGKUNGAN ATAS", top: 0xD9252B },
            { text:"BIRU DI LENGKUNGAN BAWAH", bottom: 0x0055A4 }
        ];
        ...
        // FUNGSI ARSIRAN YANG RAPI (Gak akan tumpang tindih)
        const createStripes = (x, y, w, h, maskKey = null) => {
            const graphics = this.add.graphics();
            graphics.lineStyle(2, 0xAAAAAA, 0.5); 
            // Posisi loop disamain semua biar garisnya nyambung sempurna
        ...
        // 4. GARIS HITAM DI ATAS SEMUANYA
        this.add.image(boardX, boardY, 'korea_garis').setDepth(20).disableInteractive();
        ...
        zoneAtas.on('pointerdown', (p, x, y, event) => { 
            event.stopPropagation(); paintZone(zoneAtas, true, stripesAtas); 
        });
        zoneBawah.on('pointerdown', (p, x, y, event) => { 
            event.stopPropagation(); paintZone(zoneBawah, true, stripesBawah); 
        });
        ...
        pBiru.on('pointerdown', () => {
            ...
            this.selectedColor = 0x0055A4; this.updateBrushColor(0x0055A4);
        ...
        // JUDUL NEGARA (BALIK PAKE KODE KAYA LEVEL 1!)
    ...
    checkWinCondition(zoneBg, zoneAtas, zoneBawah) {
        ...
        if (zoneBg.getData('colorCode') === 0xFFFFFF && zoneAtas.getData('colorCode') === 0xD9252B && zoneBawah.getData('colorCode') === 0x0055A4) {
            this.gameOver = true; 
