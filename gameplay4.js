class gameplayScene4 extends Phaser.Scene {
    ...
    preload() {
        ...
        // LOAD 3 WARNA PALET UNTUK ITALIA (Hijau, Putih, Merah)
        this.load.image('paletMerah', 'asset/paletmerah.png'); 
        this.load.image('paletPutih', 'asset/paletputih.png');
        this.load.image('paletHijau', 'asset/palethijau.png'); // <-- PASTIKAN ASET INI ADA
        ...
        // ASET KHUSUS LEVEL 4 (ITALIA)
        this.load.image('flagwin_lvl4', 'asset/flagwin_lvl4.png');
    ...
    create() {
        ...
        // JAWABAN BENAR UNTUK ITALIA (Hijau Kiri, Putih Tengah, Merah Kanan)
        this.flagAnswers = [
            { text:"HIJAU DI BAGIAN KIRI", left: 0x009246 },
        ...
        pHijau.on('pointerdown', () => {
            ...
            this.selectedColor = 0x009246; // Hijau Italia
            this.updateBrushColor(0x009246);
        ...
        this.add.text(titleX, titleY, 'ITALIA', { fontSize: (32 * titleScale) + 'px', fontFamily: 'Arial', color: '#000000', fontStyle: 'bold' }).setOrigin(0.5);
    ...
    checkWinCondition(zoneLeft, zoneMid, zoneRight) {
        ...
        // LOGIKA MENANG ITALIA (Hijau, Putih, Merah)
        if (leftColor === 0x009246 && midColor === 0xFFFFFF && rightColor === 0xD9252B) {
            this.gameOver = true; 
    ...
    showWinScreen() {
        ...
        // REWARD LEVEL 4
        let rewardLevel4 = localStorage.getItem('rewardLevel4');
        if(!rewardLevel4){
            let hintData = localStorage.getItem('hintData');
            let hint = hintData ? parseInt(hintData) : 0;
            hint += 1;
            localStorage.setItem('hintData', hint);
            localStorage.setItem('rewardLevel4', true);
        }
        ...
        // TAMPILKAN BENDERA ITALIA
        const flag = this.add.image(flagX, flagY, 'flagwin_lvl4').setDepth(201).setScale(flagScale * 0.8).setAlpha(0);
        ...
        // UPDATE LEVEL DATA: BUKA LEVEL 5
        const unlockNextLevel = () => {
            let levelDataStr = localStorage.getItem('levelData');
            let levelData = levelDataStr ? JSON.parse(levelDataStr) : {};
            levelData[4] = 2; // Level 4 tamat
            if (levelData[5] !== 2) levelData[5] = 1; // Level 5 terbuka (Jepang)
        ...
        next.on('pointerdown', () => { unlockNextLevel(); this.scene.start('gameplay5'); });
    }
