class gameplayScene8 extends Phaser.Scene {
    constructor() {
        super('gameplay8');
    }
    ...
    preload() {
        ...
        // PALET TURKI: Merah dan Putih aja
        ...
        this.load.image('flagwin_lvl8', 'asset/flagwin_lvl8.png');
        
        // CUMA 2 ASET INI DARI FIGMA LU
    ...
    create() {
        ...
        // JAWABAN BENAR TURKI (Merah Tua Khas Turki: 0xE30A17)
        this.flagAnswers = [
            { text:"MERAH TUA DI BAGIAN LATAR BENDERA", bg: 0xE30A17 },
            { text:"PUTIH DI BAGIAN BULAN DAN BINTANG", logo: 0xFFFFFF }
        ];
        ...
        this.hintQuestions = [
            {q:"Apa ibu kota negara Turki?",a:"Istanbul",b:"Ankara",c:"Izmir",d:"Bursa",correct:"b"},
            {q:"Bangunan bersejarah yang terkenal di Turki adalah?",a:"Colosseum",b:"Taj Mahal",c:"Hagia Sophia",d:"Piramida",correct:"c"},
            {q:"Turki berada di antara dua benua, yaitu?",a:"Asia & Australia",b:"Eropa & Afrika",c:"Asia & Eropa",d:"Amerika & Eropa",correct:"c"},
            {q:"Makanan khas Turki berupa daging panggang gulung adalah?",a:"Kebab",b:"Sushi",c:"Rendang",d:"Dimsum",correct:"a"},
      ...
      // ================= PENYUSUNAN LAYER TURKI ================= //
        ...
        const zoneLogo = this.add.image(boardX, boardY, 'turki_logo')
            .setInteractive({ pixelPerfect: true }).setAlpha(0.01).setDepth(15);

        // 3. GARIS OUTLINE DARI FIGMA LU
        this.add.image(boardX, boardY, 'turki_garis').setDepth(20).disableInteractive();
        ...
        // LOGIKA MEWARNAI (Murni Masking Gambar)
            ...
            const startPainting = () => {
                ...
                this.tweens.add({
                    ...
                    onComplete: () => {
                        ...
                        this.checkWinCondition(zoneBg, zoneLogo);
                    }
                });
            };
        };

        // EVENT KLIK (Stop propagation)
        zoneBg.on('pointerdown', (p, x, y, event) => { 
            event.stopPropagation(); paintZone(zoneBg, false, stripesBg); 
        });
        zoneLogo.on('pointerdown', (p, x, y, event) => { 
            event.stopPropagation(); paintZone(zoneLogo, true, stripesLogo); 
        });
        ...
        // PALET WARNA (Merah Turki 0xE30A17 & Putih)
        ...
        pPutih.on('pointerdown', () => {
            if (this.gameOver || this.isAnimating) return;
            this.playGlobalSFX('pop', { volume: 0.6 }); 
            this.selectedColor = 0xFFFFFF; this.updateBrushColor(0xFFFFFF);
            this.tweens.add({ targets: pPutih, scale: 0.9, duration: 100, yoyo: true });
        });

        // KODE JUDUL NEGARA (TEKS MURNI)
        ...
        this.add.text(titleX, titleY, 'TURKI', { fontSize: (32 * titleScale) + 'px', fontFamily: 'Arial', color: '#000000', fontStyle: 'bold' }).setOrigin(0.5);
    ...
    checkWinCondition(zoneBg, zoneLogo) {
        if (this.gameOver) return;
        // TURKI: BG Merah Tua (0xE30A17), Logo Putih (0xFFFFFF)
        if (zoneBg.getData('colorCode') === 0xE30A17 && zoneLogo.getData('colorCode') === 0xFFFFFF) {
            this.gameOver = true; 
    ...
    showWinScreen() {
        ...
        let rewardLevel8 = localStorage.getItem('rewardLevel8');
        if(!rewardLevel8){
            let hintData = localStorage.getItem('hintData');
            let hint = hintData ? parseInt(hintData) : 0;
            hint += 1;
            localStorage.setItem('hintData', hint);
            localStorage.setItem('rewardLevel8', true);
        }
        ...
        const flag = this.add.image(flagX, flagY, 'flagwin_lvl8').setDepth(201).setScale(flagScale * 0.8).setAlpha(0);
        ...
        const unlockNextLevel = () => {
            ...
            levelData[8] = 2; 
            if (levelData[9] !== 2) levelData[9] = 1; 
        ...
        next.on('pointerdown', () => { unlockNextLevel(); this.scene.start('gameplay9'); }); // Lanjut Level 9
    }
