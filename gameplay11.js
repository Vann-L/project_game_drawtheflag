class gameplayScene11 extends Phaser.Scene {
    constructor() {
        super('gameplay11');
    }
    ...
    preload() {
        ...
        // PALET SPANYOL: Sekarang warnanya CERAH/VIBRANT sesuai request
        ...
        this.load.image('flagwin_lvl11', 'asset/flagwin_lvl11.png');
        
        // ASET LOGO SPANYOL LU (Pastiin garis transparan, warna full color)
    ...
    create() {
        ...
        // DEFINISI WARNA VIBRANT (SESUAI GAMBAR REQUEST)
        // Kita simpan di variabel biar gampang dipanggil
        this.colorVibrantRed = 0xFF0000;    // Merah Cabe Cerah
        this.colorVibrantYellow = 0xFFEE00; // Kuning Terang Banget

        // JAWABAN BENAR SPANYOL (Pake kode warna baru yang cerah)
        this.flagAnswers = [
            { text:"MERAH DI ATAS DAN BAWAH", top: this.colorVibrantRed, bottom: this.colorVibrantRed },
            { text:"KUNING DI TENGAH", mid: this.colorVibrantYellow } 
        ];
        ...
        this.hintQuestions = [
            {q:"Apa ibu kota negara Spanyol?",a:"Barcelona",b:"Madrid",c:"Sevilla",d:"Valencia",correct:"b"},
            {q:"Tarian tradisional khas Spanyol adalah?",a:"Tango",b:"Salsa",c:"Flamenco",d:"Samba",correct:"c"},
            {q:"Olahraga tradisional Spanyol yang berhadapan dengan banteng disebut?",a:"Rodeo",b:"Matador",c:"Karate",d:"Polo",correct:"b"},
            {q:"Spanyol terletak di benua apa?",a:"Amerika",b:"Asia",c:"Afrika",d:"Eropa",correct:"d"},
            {q:"Klub sepak bola terkenal dari ibu kota Spanyol adalah?",a:"Real Madrid",b:"Barcelona",c:"Juventus",d:"Bayern Munchen",correct:"a"}
        ];
        ...
        // RUMUS SPANYOL: Merah (1/4), Kuning (2/4), Merah (1/4)
        const stripeH = flagH / 4; 
        ...
        // ================= PENYUSUNAN LAYER BENDERA SPANYOL ================= //

        // ATAS (Merah)
        const topY = boardY - flagH/2 + stripeH/2;
        ...
        // TENGAH (Kuning)
        const midY = boardY;
        const stripesMid = createStripes(boardX, midY, flagW, stripeH * 2).setDepth(11);
        const zoneMid = this.add.rectangle(boardX, midY, flagW, stripeH * 2, 0xFFFFFF)
            .setInteractive({ useHandCursor: true }).setAlpha(0.01).setDepth(10); 

        // BAWAH (Merah)
        const bottomY = boardY + flagH/2 - stripeH/2;
        ...
        // LOGO SPANYOL EFEK MAGIC
        // Layer 1 (Depth rendah): Logo Berwarna (Sembunyi dulu)
        this.logoWarna = this.add.image(boardX, boardY, 'spain_logo_warna').setDepth(16).setAlpha(0).disableInteractive();
        
        // Layer 2 (Depth tinggi): Logo Garis (Nongol terus)
        this.logoGaris = this.add.image(boardX, boardY, 'spain_logo_garis').setDepth(20).disableInteractive();

        // OUTLINE & GARIS PEMISAH (Paling Atas)
        this.add.rectangle(boardX, boardY - stripeH, flagW, 3, 0x000000).setDepth(21);
        this.add.rectangle(boardX, boardY + stripeH, flagW, 3, 0x000000).setDepth(21);
        ...
        // LOGIKA MEWARNAI (Udah diupdate logikanya)
        const paintZone = (zone, stripesToDestroy = null, isMidZone = false) => {
            ...
            const paintColor = this.selectedColor; // Warna yang nempel di kuas
            ...
            const startPainting = () => {
                ...
                this.tweens.add({
                    ...
                    onComplete: () => {
                        ...
                        // --- LOGIKA BARU UNTUK REVEAL LOGO WARNA --- //
                        if (isMidZone) {
                            // Cek apakah warna yang dipake mewarnai (paintColor) adalah Kuning Spanyol yang bener
                            if (paintColor === this.colorVibrantYellow) {
                                // JIKA BENAR: Munculin Logo Berwarna
                                this.tweens.add({ targets: this.logoWarna, alpha: 1, duration: 500, ease: 'Power2' });
                            } else {
                                // JIKA SALAH (misal diwarnai merah): Pastiin Logo Berwarna tetep ilang
                                // Biar keliatannya cuma backgroundnya berubah merah, logonya tetep garis biasa
        ...
        // PALET WARNA (Setel warna CERAH saat diklik)
        ...
        const pKuning = this.add.image(width - 90, height/1.8 + 10, 'paletKuning').setInteractive().setScale(0.8);
        pKuning.on('pointerdown', () => {
            if (this.gameOver || this.isAnimating) return;
            this.playGlobalSFX('pop', { volume: 0.6 }); 
            // Pake variabel warna cerah
            this.selectedColor = this.colorVibrantYellow; this.updateBrushColor(this.colorVibrantYellow);
        ...
        this.add.text(titleX, titleY, 'SPANYOL', { fontSize: (32 * titleScale) + 'px', fontFamily: 'Arial', color: '#000000', fontStyle: 'bold' }).setOrigin(0.5);
    ...
    checkWinCondition(zoneTop, zoneMid, zoneBottom) {
        ...
        // Pake variabel warna cerah buat ngecek kemenangan
        if (zoneTop.getData('colorCode') === this.colorVibrantRed && zoneMid.getData('colorCode') === this.colorVibrantYellow && zoneBottom.getData('colorCode') === this.colorVibrantRed) {
            this.gameOver = true; 
    ...
    showWinScreen() {
        ...
        let rewardLevel11 = localStorage.getItem('rewardLevel11');
        if(!rewardLevel11){
            let hintData = localStorage.getItem('hintData');
            let hint = hintData ? parseInt(hintData) : 0;
            hint += 1;
            localStorage.setItem('hintData', hint);
            localStorage.setItem('rewardLevel11', true);
        }
        ...
        const flag = this.add.image(flagX, flagY, 'flagwin_lvl11').setDepth(201).setScale(flagScale * 0.8).setAlpha(0);
        ...
        const unlockNextLevel = () => {
            ...
            levelData[11] = 2; 
            if (levelData[12] !== 2) levelData[12] = 1; 
        ...
        next.on('pointerdown', () => { unlockNextLevel(); this.scene.start('gameplay12'); }); 
    }
