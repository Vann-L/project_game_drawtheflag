class gameplayScene3 extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('paletBiru', 'asset/paletbiru.png'); // <-- PASTIKAN ASET INI ADA
        ...
        // ASET KHUSUS LEVEL 3 (PRANCIS)
        this.load.image('flagwin_lvl3', 'asset/flagwin_lvl3.png');
    ...
    create() {
        ...
        // JAWABAN BENAR UNTUK PRANCIS (Biru Kiri, Putih Tengah, Merah Kanan)
        this.flagAnswers = [
            { text:"BIRU DI BAGIAN KIRI", left: 0x0055A4 },
            { text:"PUTIH DI BAGIAN TENGAH", middle: 0xFFFFFF },
            { text:"MERAH DI BAGIAN KANAN", right: 0xD9252B }
        ];
        ...
        const flagW = 512; 
        const flagH = 306; 
        const thirdWidth = flagW / 3; // DIBAGI 3 VERTIKAL
        ...
        // ZONA KIRI
        const leftX = boardX - thirdWidth;
        const stripesLeft = createStripes(leftX, boardY, thirdWidth, flagH);
        const zoneLeft = this.add.rectangle(leftX, boardY, thirdWidth, flagH, 0xFFFFFF)
            .setInteractive({ useHandCursor: true }).setAlpha(0.01); 

        // ZONA TENGAH
        const midX = boardX;
        const stripesMid = createStripes(midX, boardY, thirdWidth, flagH);
        const zoneMid = this.add.rectangle(midX, boardY, thirdWidth, flagH, 0xFFFFFF)
            .setInteractive({ useHandCursor: true }).setAlpha(0.01); 

        // ZONA KANAN
        const rightX = boardX + thirdWidth;
        const stripesRight = createStripes(rightX, boardY, thirdWidth, flagH);
        const zoneRight = this.add.rectangle(rightX, boardY, thirdWidth, flagH, 0xFFFFFF)
            .setInteractive({ useHandCursor: true }).setAlpha(0.01);

        // GARIS PEMISAH HITAM (Biar kelihatan jelas potongannya)
        this.add.rectangle(boardX - thirdWidth/2, boardY, 3, flagH, 0x000000).setDepth(20);
        this.add.rectangle(boardX + thirdWidth/2, boardY, 3, flagH, 0x000000).setDepth(20);
        ...
        const paintZone = (zone, stripesObj) => {
            ...
            // Karena potongannya vertikal, arah kuasnya dimodif dikit
            ...
            this.tweens.add({
                targets: this.brushContainer, x: startX + 20, y: startY - 20, angle: -20, duration: 400, ease: 'Power2',
            ...
            const startPainting = () => {
                ...
                this.tweens.add({
                    ...
                    onUpdate: () => {
                        ...
                        // Animasi ngewarnain dari bawah ke atas
                        const currentY = startY - (animData.progress * zoneH);
                        const wobble = Math.sin(animData.progress * 20) * 15;
                        const currentX = zone.x + wobble;

                        paintGraphics.fillRect(zone.x - zoneW/2, currentY, zoneW, zoneH);
                        paintGraphics.fillCircle(currentX, currentY, zoneW / 2 + 10);

                        this.brushContainer.x = currentX + 30;
                        this.brushContainer.y = currentY - 40;
                        this.brushContainer.setAngle(-20 + Math.cos(animData.progress * 25) * 10);
                    },
                    onComplete: () => {
                        sfx.stop();
        ...
        // TATA LETAK 3 PALET WARNA DI KANAN
        ...
        pBiru.on('pointerdown', () => {
            ...
            this.selectedColor = 0x0055A4; // Biru Prancis
            this.updateBrushColor(0x0055A4);
        ...
        this.add.text(titleX, titleY, 'PRANCIS', { fontSize: (32 * titleScale) + 'px', fontFamily: 'Arial', color: '#000000', fontStyle: 'bold' }).setOrigin(0.5);
    ...
    checkWinCondition(zoneLeft, zoneMid, zoneRight) {
        if (this.gameOver) return;
        const leftColor = zoneLeft.getData('colorCode');
        const midColor = zoneMid.getData('colorCode');
        const rightColor = zoneRight.getData('colorCode');

        // LOGIKA MENANG PRANCIS (Biru, Putih, Merah)
        if (leftColor === 0x0055A4 && midColor === 0xFFFFFF && rightColor === 0xD9252B) {
            this.gameOver = true; 
    ...
    showWinScreen() {
        ...
        // REWARD LEVEL 3
        let rewardLevel3 = localStorage.getItem('rewardLevel3');
        if(!rewardLevel3){
            let hintData = localStorage.getItem('hintData');
            let hint = hintData ? parseInt(hintData) : 0;
            hint += 1;
            localStorage.setItem('hintData', hint);
            localStorage.setItem('rewardLevel3', true);
        }
        ...
        // TAMPILKAN BENDERA PRANCIS
        const flag = this.add.image(flagX, flagY, 'flagwin_lvl3').setDepth(201).setScale(flagScale * 0.8).setAlpha(0);
        ...
        // UPDATE LEVEL DATA: BUKA LEVEL 4
        const unlockNextLevel = () => {
            let levelDataStr = localStorage.getItem('levelData');
            let levelData = levelDataStr ? JSON.parse(levelDataStr) : {};
            levelData[3] = 2; // Level 3 tamat
            if (levelData[4] !== 2) levelData[4] = 1; // Level 4 terbuka (Italia)
        ...
        next.on('pointerdown', () => { unlockNextLevel(); this.scene.start('gameplay4'); });
    }
    ...
    showHintAnswer(){
        ...
        // SESUAIKAN LOGIKA HINT UNTUK KIRI TENGAH KANAN
        if(hint.left){ this.selectedColor = hint.left; this.updateBrushColor(hint.left); }
        if(hint.middle){ this.selectedColor = hint.middle; this.updateBrushColor(hint.middle); }
