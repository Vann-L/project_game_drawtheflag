class gameplayScene7 extends Phaser.Scene {
    constructor() {
        super('gameplay7');
    }
    ...
    preload() {
        ...
        // PALET SWISS: Murni Merah & Putih aja
        ...
        this.load.image('flagwin_lvl7', 'asset/flagwin_lvl7.png');
        
        // CUMA 2 ASET INI DARI FIGMA LU (Latar murni pake kode)
    ...
    create() {
        ...
        // JAWABAN BENAR SWISS
        this.flagAnswers = [
            { text:"MERAH DI BAGIAN LATAR BENDERA", bg: 0xD9252B },
            { text:"PUTIH DI BAGIAN PALANG TENGAH", cross: 0xFFFFFF }
        ];
        ...
        this.hintQuestions = [
            {q:"Apa ibu kota negara Swiss?",a:"Paris",b:"Bern",c:"Zurich",d:"Jenewa",correct:"b"},
            {q:"Negara Swiss terkenal dengan produksi apa?",a:"Jam tangan & Cokelat",b:"Mobil & Motor",c:"Kopi & Teh",d:"Baju & Sepatu",correct:"a"},
            {q:"Pegunungan terkenal yang ada di Swiss adalah?",a:"Himalaya",b:"Alpen",c:"Jayawijaya",d:"Andes",correct:"b"},
        ...
        const zoneCross = this.add.image(boardX, boardY, 'swiss_cross')
            .setInteractive({ pixelPerfect: true }).setAlpha(0.01).setDepth(15);

        // 3. GARIS OUTLINE DARI FIGMA LU
        this.add.image(boardX, boardY, 'swiss_garis').setDepth(20).disableInteractive();
            ...
            const startPainting = () => {
                ...
                this.tweens.add({
                    ...
                    onComplete: () => {
                        ...
                        this.checkWinCondition(zoneBg, zoneCross);
                    }
                });
            };
        };
        ...
        zoneCross.on('pointerdown', (p, x, y, event) => { 
            event.stopPropagation(); paintZone(zoneCross, true, stripesCross); 
        });
        ...
        // PALET WARNA (Merah & Putih aja, posisi disamain persis kaya Level 1)
        ...
        // KODE JUDUL NEGARA (Pake Teks Kode persis level 1)
        ...
        this.add.text(titleX, titleY, 'SWISS', { fontSize: (32 * titleScale) + 'px', fontFamily: 'Arial', color: '#000000', fontStyle: 'bold' }).setOrigin(0.5);
    ...
    checkWinCondition(zoneBg, zoneCross) {
        if (this.gameOver) return;
        // SWISS: BG MERAH, CROSS PUTIH
        if (zoneBg.getData('colorCode') === 0xD9252B && zoneCross.getData('colorCode') === 0xFFFFFF) {
            this.gameOver = true; 
    ...
    showWinScreen() {
        ...
        let rewardLevel7 = localStorage.getItem('rewardLevel7');
        if(!rewardLevel7){
            let hintData = localStorage.getItem('hintData');
            let hint = hintData ? parseInt(hintData) : 0;
            hint += 1;
            localStorage.setItem('hintData', hint);
            localStorage.setItem('rewardLevel7', true);
        }
        ...
        const flag = this.add.image(flagX, flagY, 'flagwin_lvl7').setDepth(201).setScale(flagScale * 0.8).setAlpha(0);
        ...
        const unlockNextLevel = () => {
            ...
            levelData[7] = 2; 
            if (levelData[8] !== 2) levelData[8] = 1; 
        ...
        next.on('pointerdown', () => { unlockNextLevel(); this.scene.start('gameplay8'); }); 
    }
    ...
    showHintAnswer(){
        ...
        if(hint.cross){ this.selectedColor = hint.cross; this.updateBrushColor(hint.cross); }
    }
