class gameplayScene17 extends Phaser.Scene {
    constructor() {
        super('gameplay17');
    }
    ...
    preload() {
        ...
        // PALET ARGENTINA: Biru Muda, Putih, Kuning
        this.load.image('paletBiruMuda', 'asset/paletbirumuda.png'); // Aset baru!
        ...
        this.load.image('flagwin_lvl17', 'asset/flagwin_lvl17.png');
        
        // ASET LOGO ARGENTINA DARI FIGMA LU
    ...
    create() {
        ...
        // Definisi Warna Argentina
        this.colorLightBlue = 0x74ACDF; // Biru Muda Argentina
        this.colorWhite = 0xFFFFFF;     // Putih
        this.colorYellow = 0xF6B40E;    // Kuning Matahari

        // JAWABAN BENAR ARGENTINA
        this.flagAnswers = [
            { text:"BIRU MUDA DI ATAS DAN BAWAH", top: this.colorLightBlue, bottom: this.colorLightBlue },
            { text:"PUTIH DI TENGAH BENDERA", mid: this.colorWhite },
            { text:"KUNING PADA MATAHARI", logo: this.colorYellow }
        ];
        ...
        // BANK SOAL ARGENTINA
        this.hintQuestions = [
            {q:"Apa ibu kota negara Argentina?",a:"Santiago",b:"Lima",c:"Buenos Aires",d:"Bogota",correct:"c"},
            {q:"Siapakah pemain sepak bola terkenal dari Argentina yang dijuluki La Pulga?",a:"Cristiano Ronaldo",b:"Neymar",c:"Diego Maradona",d:"Lionel Messi",correct:"d"},
            {q:"Argentina terletak di benua apa?",a:"Eropa",b:"Amerika Utara",c:"Amerika Selatan",d:"Afrika",correct:"c"},
            {q:"Tarian tradisional yang berasal dari Argentina adalah?",a:"Salsa",b:"Tango",c:"Flamenco",d:"Samba",correct:"b"},
            {q:"Bahasa resmi yang digunakan di Argentina adalah?",a:"Inggris",b:"Portugis",c:"Spanyol",d:"Italia",correct:"c"}
        ];
        ...
        // RUMUS ARGENTINA: Dibagi 3 horizontal rata
        ...
        // ================= PENYUSUNAN LAYER BENDERA ARGENTINA ================= //
        ...
        // LOGO MATAHARI (ZONA MEWARNAI) - Ditaruh di atas zona tengah
        const stripesLogo = createStripes(boardX, boardY, flagW, flagH, 'argentina_logo_warna').setDepth(16);
        const zoneLogo = this.add.image(boardX, boardY, 'argentina_logo_warna')
            .setInteractive({ pixelPerfect: true }).setAlpha(0.01).setDepth(15);
        
        // GARIS OUTLINE MATAHARI (Nongol terus di paling atas)
        this.add.image(boardX, boardY, 'argentina_logo_garis').setDepth(20).disableInteractive();
        ...
        // LOGIKA MEWARNAI (Animasi Kiri ke Kanan karena garisnya mendatar)
            ...
            const startPainting = () => {
                ...
                this.tweens.add({
                    ...
                    onComplete: () => {
                        ...
                        this.checkWinCondition(zoneTop, zoneMid, zoneBottom, zoneLogo);
                    }
                });
            };
        };
        ...
        // EVENT KLIK 
        zoneTop.on('pointerdown', (p, x, y, event) => { event.stopPropagation(); paintZone(zoneTop, false, stripesTop); });
        zoneMid.on('pointerdown', (p, x, y, event) => { event.stopPropagation(); paintZone(zoneMid, false, stripesMid); });
        zoneBottom.on('pointerdown', (p, x, y, event) => { event.stopPropagation(); paintZone(zoneBottom, false, stripesBottom); });
        ...
        // PALET WARNA (Biru Muda, Putih, Kuning)
        const pBiruMuda = this.add.image(width - 90, height/2.1, 'paletBiruMuda').setInteractive().setScale(0.8);
        pBiruMuda.on('pointerdown', () => {
            if (this.gameOver || this.isAnimating) return; 
            this.playGlobalSFX('pop', { volume: 0.6 }); 
            this.selectedColor = this.colorLightBlue; this.updateBrushColor(this.colorLightBlue);
            this.tweens.add({ targets: pBiruMuda, scale: 0.9, duration: 100, yoyo: true });
        });
        ...
        const pKuning = this.add.image(width - 90, height/1.5, 'paletKuning').setInteractive().setScale(0.8);
        pKuning.on('pointerdown', () => {
            if (this.gameOver || this.isAnimating) return; 
            this.playGlobalSFX('pop', { volume: 0.6 }); 
            this.selectedColor = this.colorYellow; this.updateBrushColor(this.colorYellow);
        ...
        this.add.text(titleX, titleY, 'ARGENTINA', { fontSize: (32 * titleScale) + 'px', fontFamily: 'Arial', color: '#000000', fontStyle: 'bold' }).setOrigin(0.5);
    ...
    checkWinCondition(zoneTop, zoneMid, zoneBottom, zoneLogo) {
        if (this.gameOver) return;
        // ARGENTINA: Biru Muda (Atas), Putih (Tengah), Biru Muda (Bawah), Kuning (Matahari)
        if (zoneTop.getData('colorCode') === this.colorLightBlue && zoneMid.getData('colorCode') === this.colorWhite && zoneBottom.getData('colorCode') === this.colorLightBlue && zoneLogo.getData('colorCode') === this.colorYellow) {
            this.gameOver = true; 
    ...
    showWinScreen() {
        ...
        let rewardLevel17 = localStorage.getItem('rewardLevel17');
        if(!rewardLevel17){
            let hintData = localStorage.getItem('hintData');
            let hint = hintData ? parseInt(hintData) : 0;
            hint += 1;
            localStorage.setItem('hintData', hint);
            localStorage.setItem('rewardLevel17', true);
        }
        ...
        const flag = this.add.image(flagX, flagY, 'flagwin_lvl17').setDepth(201).setScale(flagScale * 0.8).setAlpha(0);
        ...
        const unlockNextLevel = () => {
            ...
            levelData[17] = 2; 
            if (levelData[18] !== 2) levelData[18] = 1; 
        ...
        next.on('pointerdown', () => { unlockNextLevel(); this.scene.start('gameplay18'); }); // Lanjut Level 18
    }
